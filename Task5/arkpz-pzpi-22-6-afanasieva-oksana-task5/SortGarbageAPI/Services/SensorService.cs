...
namespace SortGarbageAPI.Services
{
    ...
    public class SensorService
    {
        ...
        /// <summary>
        /// Retrieves the maximum size of a container associated with a specific sensor
        /// </summary>
        /// <param name="id">The ID of the sensor</param>
        /// <returns>The container's maximum size if the sensor and container exist, null otherwise</returns>
        public async Task<float?> GetContainerBySensorIdAsync(int id)
        {
            var sensor = await _dbContext.Sensors.Include(s => s.Container)
        ...
        public async Task<List<int>> GetAllSensorIdsAsync()
        {
            var sensors = await GetAllSensorsAsync();
            return sensors.Select(sensor => sensor.SensorId).ToList();
        }
