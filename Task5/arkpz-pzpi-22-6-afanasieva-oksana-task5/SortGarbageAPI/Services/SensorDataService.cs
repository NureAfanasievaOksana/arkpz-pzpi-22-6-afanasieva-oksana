...
namespace SortGarbageAPI.Services
{
    ...
    public class SensorDataService
    {
        ...
        public SensorDataService(SortGarbageDbContext dbContext, NotificationService notificationService, EmailService emailService, IConfiguration configuration)
        {
        ...
        public async Task<SensorData> CreateSensorDataAsync(SensorData sensorData)
        {
            ...
            if (maxAbsoluteHumidity == 0)
            {
                throw new Exception("Invalid maximum absolute humidity value.");
            }
            ...
            var sensor = await _dbContext.Sensors
                .Include(s => s.Container).ThenInclude(c => c.User)
            ...
            {
                throw new Exception("Container is not associated with a user.");
            }
            string userEmail = container.User.Email;
            ...
            if (sensorData.Fullness >= _criticalFullnessThreshold)
            {
                ...
                await _emailService.SendEmailAsync(userEmail, notification.Subject, notification.Message);
